...
function formula3(n, v0, v1, v2) {
  // Watch out for replacing the replacement. e.g. replace('A', 'B').replace('B', 'C') replaces A with C.
  // That's why the formulas use lower case.
  return formulaString((n << 8) | n).replace('a', v0).replace('b', v1).replace('c', v2); // n << 8 | n makes the formula not depend on D, i.e. 3-term formula
}

function formula4(n, v0, v1, v2, v3) {
  return formulaString(n).replace('a', v0).replace('b', v1).replace('c', v2).replace('d', v3);
}
...
// 1010101010101010 indicates 1 output if A is set, i.e. f(A, B, C, D) = A.
// Output is a string e.g. "a + ~b * c"
...
function formulaString(table) {
  ...
  // Inputs: query: [A, B, C, D] values 1, 0, or -1 (don't care)
  //   terms: number of terms to add
  //   index: the position in query to start updating. E.e. index == 2 indicates that A and B are set but C and D can be modified
...
// A query is a vector [A, B, C, D] where 1 indicates the variable must be 1, 0 indicates the variable must be 0, and -1 indicates don't care.
function generatePotentialTable(query) {
  const A = 0xaaaa; // Truth table for f(A, B, C, D) = A
  const B = 0xcccc; // Truth table for f(A, B, C, D) = B
  const C = 0xf0f0; // Truth table for f(A, B, C, D) = C
  const D = 0xff00; // Truth table for f(A, B, C, D) = D
  const terms = [A, B, C, D];
